...
// editorDelRow() looks a lot like editorRowDelChar(), because in both cases we
// are deleting a single element from an array of elements by its index.
...
void editorRowInsertChar(erow *row, int at, int c) {
  // validate where we want to insert
    ...
    if (filerow >= E.numrows) {
      ...
      for (j = 0; j < len; j++) {
        if (iscntrl(c[j])) {
          ...
          // reset colors and formatting after esc[m
          if (current_color != -1) {
            char buf[16];
...
void editorDrawStatusBar(struct abuf *ab) {
  abAppend(ab, "\x1b[7m", 4); // m command => Select Graphic Rendition
